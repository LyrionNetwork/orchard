...
impl ValueSum {
    ...
    /// Returns the net value as an i64.
    ///
    /// This method exposes the internal value for external accounting logic
    /// (e.g. fee calculation in Lyrion). For a valid Bundle, this value
    /// must fit within an i64.
    pub fn net_value(&self) -> i64 {
        i64::try_from(self.0).unwrap_or_else(|_| {
             // Fallback for extreme cases to avoid panic in production
             if self.0 > 0 { i64::MAX } else { i64::MIN }
        })
    }
}
